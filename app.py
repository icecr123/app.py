...
def process_data(ledger_file, payment_file, order_file, detail_file, policy_file):
    # 1. 读取所有文件
        # 1. 读取所有文件
       # 使用 calamine 引擎，同时支持 .xls 和 .xlsx，速度更快
    df_ledger = pd.read_excel(ledger_file, dtype=str, engine='calamine')
    df_payment_raw = pd.read_excel(payment_file, dtype=str, engine='calamine')
    df_order = pd.read_excel(order_file, dtype=str, engine='calamine')
    df_detail = pd.read_excel(detail_file, dtype=str, engine='calamine')
    df_policy_raw = pd.read_excel(policy_file, dtype=str, engine='calamine')
